...
const MobileNav: FunctionComponent<IMobileNavProps> = ({user, isAuthenticated, loading, onLogout}) => {
                     ...
                     >
                        {headerLinks.map((link) => (
                           <Link
                              key={link.title}
                              to={link.href}
                              className="mb-4 py-2 pr-4 text-xl font-bold tracking-widest text-gray-900 outline outline-0 hover:text-primary-500 dark:text-gray-100 dark:hover:text-primary-400"
                              onClick={onToggleNav}
                           >
                              {link.title}
                           </Link>
                        ...
                        {isAuthenticated === true && (
                           <form onClick={onLogout}>
                              <button type="button" className="mb-4 py-2 pr-4 text-xl font-bold tracking-widest text-gray-900 outline outline-0 hover:text-primary-500 dark:text-gray-100 dark:hover:text-primary-400">
                                 Logout
                              </button>
                           </form>
                        )}
